---
title: Manobras espaciais para alcançar e explorar um asteroide triplo
date: 2022-07-11
status: publish
author: Joice Ferreira
categories:
- News
---


O que é a pesquisa?

O sistema de asteroides 2001SN263 foi descoberto em fevereiro de 2008. Ele é composto por três corpos: um central (α), de maior [massa](https://canalciencia.ibict.br/ciencia-em-sintese1/ciencias-exatas-e-da-terra/335-manobras-espaciais-para-alcancar-e-explorar-um-asteroide-triplo#), e dois secundários (β e γ), estando esses dois sob influência da [força da gravidade](https://canalciencia.ibict.br/ciencia-em-sintese1/ciencias-exatas-e-da-terra/335-manobras-espaciais-para-alcancar-e-explorar-um-asteroide-triplo#) do asteroide central. O trabalho desenvolvido pelos pesquisadores Jorge Kennety Formiga, do Instituto de Ciências e Tecnologia de São José dos Campos (Unesp/ICT), Denilson Paulo dos Santos, da Universidade Paulista de São João da Boa Vista (Unesp/SJBV), e Antônio de Almeida Prado, do Instituto Nacional de Pesquisas Espaciais (Inpe), avaliou a possibilidade de enviar um satélite da Terra para encontrar o sistema. Para isso, levou em conta a capacidade de utilizar a gravidade daqueles corpos para definir a rota do equipamento lançado. Além disso, foram analisados dois modelos matemáticos, um mais simplificado e outro mais realista, a fim de comparar a precisão de ambos.

## Como é feita a pesquisa?

Foram realizados cálculos para simular as mudanças na velocidade do satélite em duas situações possíveis: uma desenvolvendo um caminho entre α e γ e outra entre α e β. Para descobrir a melhor maneira de se fazer isso, a pesquisa comparou dois modelos matemáticos distintos, um chamado “[*patched conics*](https://canalciencia.ibict.br/pesquisa/0319_Manobras_espaciais_para_alcancar_explorar_asteroide_triplo)” e outro chamado “problema restrito dos três corpos”. No primeiro, são consideradas as interações do satélite com um asteroide de cada vez (isso faz os cálculos ficarem mais simples). No segundo modelo, mais realista, é considerado que dois asteroides formam um sistema em que ambos exercem força entre si e sobre o satélite. Os resultados mostraram que os modelos apresentaram pouca diferença em certas regiões de interesse, e que o uso do “patched conics” para simplificar o cálculo mostrou boas estimativas em algumas condições, como em dadas distâncias dos asteroides e em determinadas velocidades de aproximação.

Representação das diferentes trajetórias analisadas no estudo (linhas brancas). Os asteroides beta e gama orbitam o asteroide central, alfa, devido à força gravitacional que este exerce. As órbitas desses asteroides estão representadas pelos círculos brancos pontilhados. Os círculos vermelhos pontilhados representam as regiões em que o satélite lançado sofreria influência dos ateroides beta ou gama. Crédito: imagem baseada no artigo sobre a pesquisa original.

## Qual a importância da pesquisa?

Realizar missões espaciais custa caro, sendo necessário muito investimento e tempo de preparação. A quantidade de energia necessária para se lançar um equipamento para a órbita da Terra é imensa, sendo determinada pela força da gravidade do nosso planeta. Além disso, como ainda não possuímos materiais para impulsionar foguetes que gerem muita energia ocupando pouco espaço, é comum que em lançamentos 85% do peso do foguete seja devido ao seu combustível.

Para enviar um satélite a grandes distâncias, um dos desafios a ser superado é a impossibilidade de se reabastecer no espaço. O combustível usado nos equipamentos espaciais é necessário para corrigir as rotas e garantir que eles cheguem aos seus destinos. Assim, quanto mais eficiente for o trajeto traçado, menos combustível precisará ser armazenado no satélite, o que tem impacto no seu tamanho, peso, performance e custo.

A pesquisa demonstrou a possibilidade de se realizar manobras e correções de rota fazendo uso da gravidade dos próprios asteroides. Assim, o satélite poderá circular pelo sistema sem efetuar grandes gastos de combustível que comprometam a missão. O estudo também foi capaz de indicar em quais momentos serão necessários mais cálculos para aumentar a precisão do trajeto.

Pesquisador(es) Responsável(eis)

[Jorge Kennety S. Formiga](mailto:jkennety@yahoo.com.br)

Instituição(ões)

[Instituto de Ciências e Tecnologia de São José dos Campos (UNESP/ICT)](http://www.ict.unesp.br/)

[Instituto Nacional de Pesquisas Espaciais (INPE)](http://www.inpe.br/)

Sugestões de leitura

[Como se formam as galáxias?](https://canalciencia.ibict.br/pesquisa/0302_Como_se_formam_as_galaxias.html)

[Astronomia brasileira desvenda mistérios da estrela mais luminosa da Via Láctea](https://canalciencia.ibict.br/pesquisa/0069-Astronomia-brasileira-estrela-mais-luminosa-eta-carinae.html)

[Astrônomos estudam química de estrelas gigantes vermelhas para entender a evolução da Grande Nuvem de Magalhães](https://canalciencia.ibict.br/pesquisa/0067-Astronomia-evolucao-quimica-estrelas-gigantes-vermelhas.html)

[Telescópio Gemini observa matéria ejetada na explosão de uma Nova](https://canalciencia.ibict.br/pesquisa/0052-Telescopio-Gemini-registra-explosao-de-Nova.html)

[Trajetórias espaciais e manobras assistidas por gravidade](https://books.google.com.br/books/about/Trajet%C3%B3rias_espaciais_e_manobras_assist.html?id=_mpQHAAACAAJ&redir_esc=y)

Fonte:  https://canalciencia.ibict.br

**Ver também:**

[Conheça como funciona o trabalho de uma OSCIP que resgata animais em situação de risco e abandono.](https://dunapress.org/2020/02/24/um-belo-gesto-aos-nossos-amigos-animais-sos-peludinhos/)

[Como estabelecer metas de estudos.](https://dunapress.org/2021/02/07/como-estabelecer-metas-de-estudos/)

 [A educação de alta qualidade já está ao alcance de todos e em qualquer lugar.](https://dunapress.org/2020/07/28/sabia-que-educacao-de-qualidade-pode-estar-a-um-clique-da-sua-mao/)

O ensino de alta qualidade já está ao alcance de todos e em qualquer lugar. Educação de Ensino em Casa, Jardins de Infância e Escolas, com cursos educacionais pré-escolar, ensino básico, fundamental e médio!

Gratuitamente, clique e comece já!

[](https://dunapress.org/edu/?ref=barrosprojectmanager)
